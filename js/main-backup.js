...
function initSmoothScroll() {
    ...
    let lastClickTime = 0;
    const clickDelay = 300; // 300ms delay between clicks
    let scrollInProgress = false;

    // Enhanced link finder - find all possible navigation links
    ...
    function bindNavigationEvents() {
        ...
        navLinks.forEach((link, index) => {
            ...
            newLink.addEventListener('click', function(e) {
                return handleSmoothScroll.call(this, e);
            });
    ...
    function handleSmoothScroll(e) {
        ...
        const targetId = link.getAttribute('href') ||
                        ...
                        link.getAttribute('onclick-target') || '';
        ...
        if (!targetElement) {
            const idWithoutHash = cleanTargetId.replace('#', '');
        ...
        if (!targetElement) {
            const nameMatch = document.querySelector(`[name="${cleanTargetId.replace('#', '')}"]`);
            if (nameMatch) {
        ...
        // Strategy 4: Text content match (for About, Publications, etc.)
        if (!targetElement) {
            const targetText = cleanTargetId.replace('#', '').toLowerCase();
    ...
    function scrollToElement(targetElement, targetId) {
        ...
        try {
            // Get navbar height with better fallback
            const navbar = document.querySelector('.navbar, .navigation, header');
            let navbarHeight = 70; // Default fallback

            if (navbar) {
                const navbarStyle = window.getComputedStyle(navbar);
                navbarHeight = navbar.offsetHeight || parseInt(navbarStyle.height) || 70;
            }

            // Calculate target position
            const targetPosition = targetElement.offsetTop - navbarHeight - 20; // 20px extra offset
            ...
            if ('scrollBehavior' in document.documentElement.style) {
                ...
                animateScroll(startPosition, targetPosition, 800, () => {
                    updateActiveNavLink(targetId);
                    scrollInProgress = false;
    ...
    // Animation fallback for older browsers
...
function initGallery() {
    ...
    galleryItems.forEach(item => {
        item.addEventListener('click', function() {
            // This can be enhanced with lightbox functionality
            const img = this.querySelector('img');
            if (img) {
...
function initShareModal() {
    // This function can be implemented when share functionality is needed
    console.log('Share modal functionality ready');
}
...
// Validate specific social icons
...
// Validate if a specific icon class exists
...
function replaceIconsWithFallback(elements, config) {
    elements.forEach(element => {
        ...
        // If no alternatives work, use text fallback
